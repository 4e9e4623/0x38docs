...
* 本プログラムについてのコンテンツ（解説の動画や記事等）を作成した場合は下記の送信専用フォームから公開後にURLの送信をお願いします（こちらからの返信はありません）
...
* 抽出対象レイヤーのテキストを含むオブジェクトのテキストがexoファイル内で表せる文字数であること
    * exoファイル内ではテキストデータは16進数表記のUTF-16 LEで終端文字を含め4096文字の固定長で表現されています
* 抽出対象レイヤーのテキストを含むオブジェクトの表示時間がexoファイルの時間内に収まっていること
* 抽出対象レイヤーのテキストを含むオブジェクトで縦書きを使用していないこと
* 抽出対象レイヤーのテキストを含むオブジェクトで横書き設定で縦書きになるように（またはその逆に）していないこと
* 抽出対象レイヤーのテキストを含むオブジェクトはShift_JIS（厳密にはCP932）とUTF-8のどちらでも表現可能な文字のみを使用していること
...
* 抽出対象レイヤーのテキストを含むオブジェクトは「ASCII制御文字」や「Unicode制御文字」を使用していないこと
...
## 使用報告について
出力ファイルを使用した際は上記の問い合わせ等のフォームから報告（任意）を頂けると励みになります  
※毎回だと面倒だと思うので、チャンネル単位で1度でも構いません  
本プログラムについてのコンテンツ（解説の動画や記事等）を作成した場合は、コンテンツ単位で上記フォームから報告をお願いします  
