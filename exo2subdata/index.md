...
* 本プログラムについてのコンテンツ（解説の動画や記事等、Twitterなどは除く）を作成した場合は下記の送信専用フォームから公開後にURLの送信をお願いします（こちらからの返信はありません）
...
## 使用報告について
前述の通り出力ファイルを使用した際の報告等は必要ありませんが、上記の問い合わせ等のフォームから報告（任意）を頂けると励みになります  
※毎回だと面倒だと思うので、チャンネル単位で1度でも構いません  
本プログラムについてのコンテンツ（解説の動画や記事等、Twitterなどは除く）を作成した場合は、コンテンツ単位で上記フォームから報告をお願いします  
